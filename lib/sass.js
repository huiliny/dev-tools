  ...
  if (path === 'directory') {
    ...
    fs.readdir('src/sass/', function (err, paths) {
      ...
      paths.forEach(function (src) {
        if (src.indexOf('_') !== 0) {
          // 不是以_开头
          const dist = 'src/static/css/' + src.replace('.scss', '.css');
    ...
    const dist = path.replace('/sass/', '/static/css/').replace('.scss', '.css');
...
function imgToBase64(content, dist, type, config) {
  let log = type === 'watch';
  const outPath = dist.replace('src/static/css', `${config.dist}/static/css`);
